...
Components.MainWindow {
    ...
    BackEnd {
        id: file
        filePath: "\"Drivers/InstallDrivers.cmd\""
    }
    ...
    required property Dialogs.SuccessInstallation nextDialog
    ...
    Connections {
        ...
        function onAppend(string) {
            ...
            if (string.includes("successful") && logger.progressBar.bar.width < logger.progressBar.parent.width)
                logger.progressBar.bar.width += logger.progressBar.width / 3
        }
        function onEnd() {
    ...
    topBanner: Constants.TopBanner {
        title.text: Languages.fromString(window.language,
                                    Languages.InstallingDriverDialogTitle)
        desc.text: Languages.fromString(window.language,
                                    Languages.InstallingDriverDialogDesc)
    }
